...
export class Register {
  ...
  static __registerTransformGroup(transformGroup, target) {
    ...
    if (!Array.isArray(transforms) || !target.hooks.transforms)
      throw new Error('transforms must be an array of registered value transforms');

    const unregisteredTransforms = transforms.filter((t) => !target.hooks.transforms[t]);
    if (unregisteredTransforms.length > 0)
      throw new Error(`transforms must be an array of registered value transforms. The following unregistered transforms were found:\n- ${unregisteredTransforms.join('\n- ')}`);
